...
class ViewController: UIViewController {
    ...
    @IBAction func startGame(sender: AnyObject) {
        ...
        textField.text = newGame.firstHand()
    }
        
    @IBAction func dealNewCard(sender: UITapGestureRecognizer) {
        if self.newGame.gameOver == false {
            textField.text = newGame.deal()
        }
    }
    
    
    @IBAction func endGame(sender: UISwipeGestureRecognizer) {
        if self.newGame.gameOver == false {
            textField.text = newGame.endGame()
        }
        self.newGame.gameOver == true
    }
