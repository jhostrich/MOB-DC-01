...
class FirstViewController: UIViewController {
    ...
    @IBAction func generateTextButton(sender: AnyObject) {
        var newLabelText = Greeting(age: 0)
        if nameField.text != "" {
            newLabelText.name = nameField.text
        } 
        newLabelText.age = ageField.text.toInt()
        
        mainLabel.text = newLabelText.helloYou() + "\n" + newLabelText.oldEnoughToComplex()
